...
def _gather_claim_data_full(db: Session, claim: Claim) -> dict[str, Any]:
    ...
    return {
        ...
        "gross_total": round(gross_total_claimed, 2) if gross_total_found else round(billed_total, 2),
        "net_payable": round(net_payable_claimed, 2) if net_payable_found else round(billed_total, 2),
        "deductions": round(max(0.0, (gross_total_claimed - net_payable_claimed)), 2) if (gross_total_found and net_payable_found) else 0.0,
